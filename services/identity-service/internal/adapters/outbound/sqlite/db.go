...
// domain.UserRepository and domain.VerificationTokenRepository. It exists
// as the local/acceptance-test-time swap-in for the postgres adapter —
// same domain contracts, no server process required. Migrations are
// embedded at compile time and are SQLite-dialect equivalents of the
// postgres migrations, not shared SQL text (see
// migrations/000001_create_users.up.sql for the dialect-translation notes).
package sqlite
...
// with foreign_keys and busy_timeout pragmas so ON DELETE CASCADE and
// light concurrent access behave correctly regardless of what the caller
// passed in. The caller is responsible for closing the returned *sql.DB.
...
// width keeps lexicographic and chronological ordering identical.
...
// nullTimeToText formats t as RFC3339Nano, or returns a NULL-valued
// sql.NullString when t is nil — for optional timestamp columns like
// email_verified_at and used_at.
...
// textToNullTime parses a nullable TEXT timestamp column written by
// nullTimeToText, returning nil when the column was NULL.
