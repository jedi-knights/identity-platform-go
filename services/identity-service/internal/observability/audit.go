...
type AuditWiring struct {
	// Emitter is the audit.Emitter that the application layer depends on.
...
// NewAuditEmitter constructs the audit emitter for identity-service per
...
func NewAuditEmitter(ctx context.Context, cfg *config.Config, log logging.Logger) (*AuditWiring, error) {
	...
	if cfg.Audit.DurableDSN == "" {
		log.Info("audit emitter configured with stderr sink only (IDENTITY_AUDIT_DURABLE_DSN not set)")
		return &AuditWiring{
...
// Sized so a typical login burst (a few hundred per second) cannot drop
// under healthy operation while staying small enough that sustained
// back-pressure surfaces quickly in [audit.AsyncSink.Stats].
