...
| `urn:ietf:params:oauth:grant-type:saml2-bearer` | Fully implemented (ADR-0026) | RFC 7522 §2.1 only (assertion as authorization grant, not §2.2 client auth). No refresh token issued. Requires the client's `TrustedIssuerCert` (from client-registry-service) to be set — otherwise `invalid_grant`. |
...
- **Step-up authentication** (RFC 9470, ADR-0024): `/oauth/authorize`'s `acr_values` is parsed and stored on `LoginChallenge.AcrValues` for protocol completeness (and advertised via `acr_values_supported` in OIDC metadata), but this platform has exactly one authentication method — every `authorization_code` redemption re-authenticates the user from scratch via login-ui, so the issued token's `domain.Token.Acr` is unconditionally `domain.AcrValuePassword` ("pwd"), regardless of what `acr_values` was requested. `acr` is never lifted onto the signed JWT — `go-platform/jwtutil.Claims` has no field for it and is an externally-versioned module this repo doesn't own — so it is only visible via `POST /oauth/introspect`'s `acr` field (`TokenService.Introspect` sources it from the stored `domain.Token`, not the JWT-validated one). A second authentication method (e.g., a real step-up UI in login-ui) would need to branch on `AcrValues` here; today there is nothing to branch to.
- **DPoP** (RFC 9449, ADR-0025): optional, per-request — a `DPoP` header at `/oauth/token` is validated by `application.DPoPValidator` (grant-agnostic, runs once in `Handler.Token` before grant dispatch) and stamps `domain.Token.JKT` + `domain.TokenTypeDPoP` on the issued token via `client_credentials`, `refresh_token`, and `authorization_code` (not `token_exchange` — stated ADR scope cut). `acr`/`cnf` never reach the signed JWT — `go-platform/jwtutil.Claims` has no field for either — so `cnf.jkt` is only visible via `POST /oauth/introspect`. The DPoP proof `jti` replay cache (`domain.DPoPProofRepository`) is "insert-if-absent, TTL'd," the opposite shape from `AuthorizationCodeRepository.Consume`'s "read-and-delete" — do not conflate the two when adding a new replay-style repository.
- **SAML 2.0 Bearer Assertion Grant** (RFC 7522, ADR-0026): `application.SAMLBearerValidator` reuses `github.com/crewjam/saml`'s schema types purely as `encoding/xml` unmarshal targets — that library has no exported entrypoint for validating a bare assertion outside a full SSO Response envelope — and calls `github.com/russellhaering/goxmldsig` directly for signature verification. This repo's `go.mod` pins `goxmldsig >= 1.6.0` explicitly (overriding crewjam/saml's own `v1.4.0` requirement) to close CVE-2026-33487, a signature-wrapping-class bypass in the exact validation function this feature depends on. **Do not downgrade this override** without confirming crewjam/saml has shipped a release requiring the fixed version. The expected assertion audience/recipient is `application.SAMLBearerStrategy`'s statically-configured token endpoint URL (from `AUTH_METADATA_PUBLIC_BASE_URL`), not a live-request-derived value — unlike DPoP's `htu` check, grant strategies have no `*http.Request` in hand.
