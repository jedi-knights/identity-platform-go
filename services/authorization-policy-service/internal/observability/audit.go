...
// NewAuditEmitter constructs the audit emitter for authorization-policy-service
...
func NewAuditEmitter(ctx context.Context, cfg *config.Config, log logging.Logger) (*AuditWiring, error) {
	...
	if cfg.Audit.DurableDSN == "" {
		log.Info("audit emitter configured with stderr sink only (POLICY_AUDIT_DURABLE_DSN not set)")
