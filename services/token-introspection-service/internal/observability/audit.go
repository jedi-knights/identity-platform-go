...
// NewAuditEmitter constructs the audit emitter for token-introspection-service
...
func NewAuditEmitter(ctx context.Context, cfg *config.Config, log logging.Logger) (*AuditWiring, error) {
	...
	if cfg.Audit.DurableDSN == "" {
		log.Info("audit emitter configured with stderr sink only (INTROSPECT_AUDIT_DURABLE_DSN not set)")
