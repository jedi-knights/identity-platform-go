...
| [RFC 9470](https://datatracker.ietf.org/doc/html/rfc9470) | Step Up Authentication Challenge — `acr_values` request parameter, `insufficient_user_authentication` challenge | `services/auth-server`, [ADR-0024](docs/adr/0024-step-up-authentication-challenge.md). This platform's one authentication method satisfies any `acr_values` request, so the AS-side round trip (`acr_values` → login-ui → issued token's `acr` → introspection) is fully real and end-to-end tested; `example-resource-service`'s `RequireACRMiddleware` is a real, unit-tested enforcement mechanism with no live `acr` data source in the standard `token-introspection-service` topology today — see the ADR's Consequences section. |
| [RFC 9449](https://datatracker.ietf.org/doc/html/rfc9449) | DPoP — Demonstrating Proof of Possession | `services/auth-server`, [ADR-0025](docs/adr/0025-dpop-proof-of-possession.md). Optional, per-request — a `DPoP` header at `/oauth/token` yields a `DPoP`-typed, key-bound token whose `cnf.jkt` is echoed by introspection; no header keeps today's unchanged Bearer behavior. `example-resource-service`'s `RequireDPoPMiddleware` is a real, unit-tested enforcement mechanism with no live `cnf.jkt` data source in the standard `token-introspection-service` topology today — see the ADR's Consequences section. |
| [RFC 7522](https://datatracker.ietf.org/doc/html/rfc7522) | SAML 2.0 Bearer Assertion Grant — exchange a SAML assertion for an access token | `services/auth-server`, `services/client-registry-service`, [ADR-0026](docs/adr/0026-saml2-bearer-assertion-grant.md). Scoped to §2.1 (assertion as authorization grant) only, not §2.2 (SAML client authentication). Trust is per-client via an optional `trusted_issuer_cert` field; uses `github.com/crewjam/saml`'s schema types with `github.com/russellhaering/goxmldsig` for signature verification (crewjam/saml has no standalone-assertion entrypoint) — this repo pins `goxmldsig >= 1.6.0` explicitly, overriding crewjam/saml's own `v1.4.0` requirement, to close CVE-2026-33487. |
